...
void initialize(Energy<StableNeoHookean<Space3D, Space3D>> &constraints,
                const Energy<StableNeoHookean<Space3D, Space3D>>::RestPoseT &x0)
{
...
gradients(const Energy<StableNeoHookean<Space3D, Space3D>> &constraints,
...
Eigen::Matrix<Space3D::Scalar, 2, 2> compliance(const Energy<StableNeoHookean<Space3D, Space3D>> &constraints, int cId)
{
    const float alphaH = 1.0F / (constraints.volumes[cId] * constraints.lambda[cId]);
    const float alphaD = 1.0F / (constraints.volumes[cId] * constraints.mu[cId]);
