#ifndef VELO_PHYSICS_SNHCONSTRAINT_H
#define VELO_PHYSICS_SNHCONSTRAINT_H
...
void VELOPHYSICS_EXPORT initialize(Energy<StableNeoHookean<Space3D, Space3D>> &constraints,
                                   const StableNeoHookean<Space3D, Space3D>::RestPoseT &x0);

std::pair<Eigen::Vector<Space3D::Scalar, Energy<StableNeoHookean<Space3D, Space3D>>::CSize>,
          Eigen::Matrix<Space3D::Scalar,
                        Energy<StableNeoHookean<Space3D, Space3D>>::NbParticles * Space3D::Dim,
                        Energy<StableNeoHookean<Space3D, Space3D>>::CSize>>
    VELOPHYSICS_EXPORT gradients(const Energy<StableNeoHookean<Space3D, Space3D>> &constraints,
                                 const StableNeoHookean<Space3D, Space3D>::PoseT &x,
                                 int cId);

Eigen::Matrix<Space3D::Scalar, 2, 2> VELOPHYSICS_EXPORT compliance(const Energy<StableNeoHookean<Space3D, Space3D>> &constraints,
                                                                   int cId);

}  // namespace velo::physics

#endif  //  VELO_PHYSICS_SNHCONSTRAINT_H